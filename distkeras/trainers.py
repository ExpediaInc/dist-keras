...
from distkeras.workers import ADAGWorkerWithDistributedParameterServer
...
class ADAGADAM(AsynchronousDistributedTrainer):
    ...
    def __init__(self, keras_model, worker_optimizer, loss, metrics=["accuracy"], num_workers=2, batch_size=32,
                 features_col="features", label_col="label", num_epoch=1, communication_window=12, master_port=5000, loss_weights=None, alpha=1e-5, beta_1=0.9, beta_2=0.999, epsilon=1e-8, worker_learning_rate=1e-5, should_return_history = False):
        # Initialize the parent object.
    ...
    def allocate_parameter_server(self):
        ...
        parameter_server = ADAGParameterServerADAM(self.master_model, self.master_port, alpha=self.a, beta_1=self.b1, beta_2=self.b2, epsilon=self.e, worker_learning_rate=self.worker_learning_rate)
        return parameter_server

class ADAGADAMPooled(AsynchronousDistributedTrainer):
    """Asynchronous Distributed Adaptive Gradient (Stochastic Gradient Descent).
...
class ADAGWithDistributedParameterServer(AsynchronousDistributedTrainer):
    """Experimental optimization scheme for development purposes."""

    def __init__(self, keras_model, worker_optimizer, loss, metrics=["accuracy"], num_workers=2, batch_size=32,
                 features_col="features", label_col="label", num_epoch=1, communication_window=12, master_port=5000, loss_weights=None,
                 num_children=3, communication_window_executor=10,num_iter_loss_avg = 1000, warm_up_iter=1, warm_up_scale = 1 ):
        # Initialize the parent object.
    ...
    def allocate_worker(self):
        ...
        worker = ADAGWorkerWithDistributedParameterServer(self.master_model, self.worker_optimizer, self.loss, self.loss_weights, self.metrics,
                            ...
                            self.num_children, self.communication_window, self.worker_ip_id, self.ip_list, self.num_iter_loss_avg,self.warm_up_iter,self.warm_up_scale)
        return worker
